...
# ------ FUNCTIONS -------------------------------------------------------------------------------------
RDMarkovMeans <- function(SFile="SMILES.txt",sResultFile="RDMarkovMeansResults.csv",kPower="3", atPropExc="Zv,EA") {
  ...
  #-------------------------------------------------------------------------------
  dfW=read.table("weight/AtomProperties.txt",header=T)  # read weigths TXT file
  print(dfW)# load("AtomProperties.RData")         # read R data -> dfW
  
  excludeAtomsFormat <-lapply(strsplit(atPropExc,','), as.character)[[1]]
  print (excludeAtomsFormat)
  outputAtomsFile<-dfW[ , !(names(dfW) %in% excludeAtomsFormat)]
  
  print(outputAtomsFile)
...
# -----------------------------------------------------------------------------------------------------------------
RDMarkovSingulars <- function(SFile="SMILES.txt",sResultFile="RDMarkovSingularsResults.csv",kPower=3, atPropExc="Zv,EA", fAllKs=0) {
